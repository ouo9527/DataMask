...
public interface DesensitizationHandler {
     ...
     * @return 返回已脱敏数据
     */
    default <T> T desensitized(String context, SceneEnum scene, String fieldName, T data) {
        if (this.supports(context)) return this.desensitized(scene, fieldName, data);
        return data;
    }
     ...
     * @return 返回已脱敏数据
     */
    default <T> T desensitized(String context, SceneEnum scene, Field field, T data) {
        if (this.supports(context)) return this.desensitized(scene, field, data);
        return data;
    }
     ...
     * @return 返回已脱敏Java Bean或Map数据
     */
    default <T> T desensitized(String context, SceneEnum scene, T data) {
        if (this.supports(context)) return this.desensitized(scene, data);
        return data;
    }
}
