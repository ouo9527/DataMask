 ...
 */
public final class DesensitizingFieldSerializer<T> extends FieldSerializer<T> {

    public DesensitizingFieldSerializer(Kryo kryo, Class type) {
        super(kryo, type);
    }
    ...
    public T copy(Kryo kryo, T original) {
        ...
        FieldSerializer.CachedField[] copyFields = this.getCopyFields();
        for (int i = 0, n = copyFields.length; i < n; i++)
            this.copy(copyFields[i], original, copy);
     ...
     * @param dest        目标对象
     */
    private void copy(FieldSerializer.CachedField cachedField, Object src, Object dest) {
        ...
        if (kryo instanceof DesensitizingKryo) {
            if (cachedField instanceof UnsafeField) {
                // 采用Unsafe处理字段脱敏
                try {
                    ((UnsafeField) cachedField).set(dest, ((DesensitizingKryo) kryo).getDesensitizer().desensitized(((DesensitizingKryo) kryo).getSceneEnum(),
                            field, ((UnsafeField) cachedField).get(src)));
                } catch (KryoException ex) {
                    ex.addTrace(this + " (" + this.getType().getName() + ")");
                    throw ex;
                } catch (Throwable t) {
                ...
                // 采用ASM处理字段脱敏
                try {
                    ((AsmField) cachedField).set(dest, ((DesensitizingKryo) kryo).getDesensitizer().desensitized(((DesensitizingKryo) kryo).getSceneEnum(),
                            field, ((AsmField) cachedField).get(src)));
                } catch (KryoException ex) {
                    ex.addTrace(this + " (" + this.getType().getName() + ")");
                    throw ex;
                } catch (Throwable t) {
                ...
                // 采用反射处理字段脱敏
                try {
                    ((ReflectField) cachedField).set(dest, ((DesensitizingKryo) kryo).getDesensitizer().desensitized(((DesensitizingKryo) kryo).getSceneEnum(),
                            field, ((ReflectField) cachedField).get(src)));
                } catch (IllegalAccessException ex) {
                    throw new KryoException("Error accessing field: " + field.getName() + " (" + this.getType().getName() + ")", ex);
                } catch (KryoException ex) {
                    ex.addTrace(field.getName() + " (" + this.getType().getName() + ")");
                    throw ex;
                } catch (Throwable t) {
                    KryoException ex = new KryoException(t);
                    ex.addTrace(field.getName() + " (" + this.getType().getName() + ")");
