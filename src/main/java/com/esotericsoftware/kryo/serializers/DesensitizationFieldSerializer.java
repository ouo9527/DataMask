...
public class DesensitizationFieldSerializer<T> extends FieldSerializer<T> {
    ...
    public T copy(Kryo kryo, T original) {
        ...
        for (int i = 0, n = cachedFields.copyFields.length; i < n; i++)
            proxy(cachedFields.copyFields[i]).copy(original, copy);
    ...
    private CachedField proxy(CachedField cachedField) {
        ...
        if (null != newCachedField) {
            newCachedField.name = cachedField.name;
            newCachedField.valueClass = cachedField.valueClass;
            newCachedField.serializer = cachedField.serializer;
            newCachedField.varEncoding = cachedField.varEncoding;
            newCachedField.canBeNull = cachedField.canBeNull;
            newCachedField.optimizePositive = cachedField.optimizePositive;
            newCachedField.reuseSerializer = cachedField.reuseSerializer;
            newCachedField.access = cachedField.access;
            newCachedField.accessIndex = cachedField.accessIndex;
            newCachedField.offset = cachedField.offset;
            newCachedField.tag = cachedField.tag;
